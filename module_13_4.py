...
async def start(message):
    ...
    await message.answer('Введите слово "Калории" для подсчета суточной нормы калорий!')

@dp.message_handler(text=['калории', 'Калории', 'КАЛОРИИ'])
...
async def send_calories(message, state):
    ...
    if data['male'] == 'ж' or 'Ж':
        ...
        await message.answer(f'Ваша норма калорий c учётом активности = {calories} (ккал)')
    elif data['male'] == 'м' or 'М':
        calories = ((10 * float(data['weight'].replace(',', '.')) + 6.25 * float(data['growth'].replace(',', '.')) -
                     5 * float(data['age'].replace(',', '.')) + 5) * float(data['activity'].replace(',', '.')))
        await message.answer(f'Ваша суточная норма калорий c учётом активности = {calories} (ккал)')
    await state.finish()
