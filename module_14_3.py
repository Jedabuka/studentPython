...
pers, text_per, price_per = 'Персик', 'Сочный фрукт с бархатистой кожицей', 100
granat, text_gran, price_gran = 'Гранат', 'Отличная замена попкорну', 200
mang, text_mang, price_mang = 'Манго', 'Райское наслаждение', 300
anan, text_anan, price_anan = 'Ананас', 'Спелый! Ваш рот останется цел', 400
...
async def get_buying_list(message):
    with open('picture/persik1.webp', 'rb') as per:
        await message.answer_photo(per, f'Название: {pers} | Описание: {text_per} | Цена: {price_per}')
    with open('picture/granat2.jpg', 'rb') as gran:
        await message.answer_photo(gran, f'Название: {granat} | Описание: {text_gran} | Цена: {price_gran}')
    with open('picture/mango3.jpg', 'rb') as man:
        await message.answer_photo(man, f'Название: {mang} | Описание: {text_mang} | Цена: {price_mang}')
    with open('picture/ananas4.jpg', 'rb') as an:
        await message.answer_photo(an, f'Название: {anan} | Описание: {text_anan} | Цена: {price_anan}')
